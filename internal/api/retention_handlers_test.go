...
func retentionRouter(t *testing.T, withSweeper bool) (http.Handler, string, string) {
	...
	router := NewRouter(st, sc, p, cm, em, NewRerankManager(st, zap.NewNop()), NewLLMManager(st, zap.NewNop()), nil, NewSyncJobManager(st, zap.NewNop()), nil, sw, nil, testJWTSecret, nil, nil, nil, zap.NewNop())
