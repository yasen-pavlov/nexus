...
func rankingRouter(t *testing.T) (http.Handler, *RankingManager, string, string) {
	...
	router := NewRouter(st, sc, p, cm, em, NewRerankManager(st, zap.NewNop()), NewLLMManager(st, zap.NewNop()), nil, nil, NewSyncJobManager(st, zap.NewNop()), nil, nil, rankingMgr, testJWTSecret, nil, nil, nil, zap.NewNop())
...
func TestUpdateRankingSettings_StoreClosedSurfacesError(t *testing.T) {
	...
	em := NewEmbeddingManager(st, zap.NewNop())
	p := pipeline.New(st, sc, em, zap.NewNop())
	router := NewRouter(st, sc, p, cm, em, NewRerankManager(st, zap.NewNop()), NewLLMManager(st, zap.NewNop()), nil, nil, NewSyncJobManager(st, zap.NewNop()), nil, nil, NewRankingManager(st, zap.NewNop()), testJWTSecret, nil, nil, nil, zap.NewNop())
